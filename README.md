...
* **Frontend:** React 18 with Vite, styled using Tailwind CSS and Framer Motion for smooth animations. The frontend uses Recharts for data visualization and HTML-to-Image for exporting the dashboard as a PNG.
...
3. **Planning & Dashboard Phase:**
   ...
   - The frontend automatically captures a screenshot of the rendered dashboard using `html-to-image` and POSTs the base64 image back to the backend at `/api/job/:jobId/dashboard-image`.
...
4. **Narrative & Report Generation Phase:**
   ...
   - Once the backend receives the dashboard image from the frontend, it combines the AI narrative and the dashboard image into a formatted PDF using `pdfkit`.
...
5. **Delivery Phase:**
   ...
   - The user can trigger an email delivery via the `EmailModal.tsx` component, which sends a request to `/api/job/:jobId/email`. The backend uses the Gmail API (via provided OAuth tokens) to securely dispatch the package.
