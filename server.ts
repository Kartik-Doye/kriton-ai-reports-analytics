 ...
 * @param retries - Maximum number of retry attempts (default: 15)
 * @param initialDelay - Initial delay in milliseconds before retry (default: 2000)
...
app.post('/api/upload', upload.single('file'), (req: Request, res: Response) => {
  try {
    ...
    // Rate limiting: max 5 uploads per IP per 10 minutes
    const ip = req.ip || 'unknown';
    const now = Date.now();
    const rateLimitKey = ip;
    const limit = uploadRateLimit.get(rateLimitKey);
    ...
    if (limit && now < limit.resetAt) {
      ...
      uploadRateLimit.set(rateLimitKey, { count: 1, resetAt: now + 10 * 60 * 1000 });
    }
    ...
    if (!req.file.buffer || req.file.buffer.length === 0) {
      return res.status(400).json({ error: 'File is empty. Please upload a valid file.' });
    }
...
app.get('/api/job/:jobId/download/:fileType', async (req: Request, res: Response) => {
  ...
  if (fileType === 'csv' && job.cleanedData) {
    ...
    const csvStr = (Papa as any).unparse(job.cleanedData);
...
app.post('/api/job/:jobId/chat', async (req: Request, res: Response) => {
  const { jobId } = req.params;
...
app.post('/api/job/:jobId/email', async (req: Request, res: Response) => {
  const { jobId } = req.params;
...
app.post('/api/job/:jobId/refresh', async (req: Request, res: Response) => {
  const { jobId } = req.params;
  const job = jobs.get(jobId);
  if (!job) return res.status(404).send('Job not found');
...
/**
 * Generates AI content with caching to avoid duplicate API calls.
 * @param prompt - The prompt to send to Gemini AI
 * @param config - Optional configuration for the AI model (e.g., response schema)
 * @returns Promise with generated text
 * @description Uses SHA256 hash of prompt as cache key. Results > 100KB are not cached.
 */
...
/**
 * Main data processing pipeline: cleans data, generates dashboard spec, and creates reports.
 * 
 * Pipeline stages:
 * 1. Parse and sample raw data
 * 2. Send to Gemini for AI cleaning plan
 * 3. Apply cleaning transformations programmatically
 * 4. Send cleaned data to Gemini for dashboard specification
 * 5. Compute KPI values and generate narrative report
 * 6. Generate PDF and HTML artifacts
 * 
 * @param job - The PipelineJob to process
 * @throws Error if any critical stage fails (e.g., file parsing with no data rows)
 * @emits SSE events to connected clients: status, log, spec, error
 */
...
/**
 * Sends a report via Gmail using the user's OAuth2 credentials.
 * @param job - The PipelineJob containing the report and user email
 * @param options - Email customization options (to, cc, bcc, subject, body, attachHtml)
 * @throws Error if user is not authenticated with Google or if Gmail API fails
 * @description Uses Gmail API v1 with the user's access token to securely send emails.
 * Always attaches the generated PDF report. Logs all steps via SSE events.
 */
async function sendEmail(job: PipelineJob, options: { to?: string, cc?: string, bcc?: string, subject?: string, body?: string, attachHtml?: boolean } = {}) {
  ...
  const MailComposer = (await import('nodemailer/lib/mail-composer/index.js' as any) as any).default || await import('nodemailer/lib/mail-composer/index.js' as any);
...
app.delete('/api/job/:jobId', (req: Request, res: Response) => {
  const { jobId } = req.params;
  const job = jobs.get(jobId);
